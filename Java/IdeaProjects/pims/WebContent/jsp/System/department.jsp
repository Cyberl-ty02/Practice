    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#departmentinfo").datagrid({
                ...
                url: 'SystemDepartmentService?caozuo=init&con='+getQueryCondition()                        	
            });	
            
	        //添加操作，弹出对话框
	    	$("#add").click(function () {
	    	    operation="add";	    		    	    	    
	    		$('#dlgdepartmentinfo').dialog('open').dialog('center').dialog('setTitle','添加部门信息');
	    		$('#fm').form('clear');
	    	});	
	    	//编辑操作,弹出对话框
	    	$("#edit").click(function () {
	    	...
	    	$("#save").click(function(){
	    		...
	    		if(operation=="add"){
	    			$.post("SystemDepartmentService?caozuo=add",
	    			$("#fm").serialize(),                        
	                function (data) {	                	
	                    var value=eval("("+data+")")
		            	if (value.ret == "0") {
	    ...
	    function getQueryCondition(){
	        var condition="";
	        if($("#qdeptcode").val()!=""){
	        	condition="deptcode='"+$("#qdeptcode").val()+"'";
	        }
	        if($("#qdeptname").val()!=""){
	        	if(condition!=""){
	        		condition=condition+" and deptname like '%"+$("#qdeptname").val()+"%'"
	        	}else{
	        		condition="deptname like '%"+$("#qdeptname").val()+"%'";
	        	}
	        }
	        if($("#qdutypeople").val()!=""){
		        var dutypeople=$("#qdutypeople").val();	        
	            if(condition!=""){
	        		condition=condition+" and dutypeople='"+dutypeople+"'"
	        	}else{
	        		condition="dutypeople='"+dutypeople+"'";
	            }		        	
	        }	               
     ...
     <div style="height:98%;">
	     ...
	     <!-- 明细及工具栏信息开始 -->	 
	 	 <table id="departmentinfo" class="easyui-datagrid" width="100%" style="height:100%;" border="0" cellpadding="0" cellspacing="0" 
	    	...
	    	<thead>
	    		<tr>   			
	    			<th data-options="field:'deptcode',width:200,align:'center'">部门编码</th>								
					<th data-options="field:'deptname',width:300,align:'left'">部门名称</th>
