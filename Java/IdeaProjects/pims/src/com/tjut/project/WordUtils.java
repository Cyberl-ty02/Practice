...
public class WordUtils {
    ...
    public static String getText(Document document) {
		...
		//for(int i = 0; i < document.getSections().getCount(); i++) {
            ...
            /*
            System.out.println(section.getTables().getCount());
            for(int j=0;j<section.getTables().getCount();j++) {
            	Table table=document.getSections().get(i).getTables().get(j);
            	System.out.println(table.getDocument().getText());
            	System.out.println("----------------------------------------------");
            }
            ...
            for (int j = 0; j < section.getParagraphs().getCount(); j++) {
                ...
                if(document.getSections().get(i).getTables().getCount()>0) {
                	for(int i1=0;i1<document.getSections().get(i).getTables().getCount();i1++) {
	                	Table table=document.getSections().get(i).getTables().get(i1);
	                	TableRow row=table.getRows().get(table.getRows().getCount()-1);  
    ...
    //获得List某行的部分数据,且以：分割的后部分（使用合同文档）;注：字符串中若有空格则删除
    //应用文档：项目合同
